...
def upload_image():
    ...
    if form.validate_on_submit():
        ...
        # 파일 DB에 저장
        user_image = UserImage(user_id=current_user.id, image_path=image_uuid_file_name, user_image_name=file.filename)
...
def search():
    ...
    for user_image in user_images:
        ...
        user_image_tag_dict[user_image.UserImage.id] = user_images_tags
...
def draw_texts(result_image, line, c1, cv2, color, labels, label):
    ...
    font = max(line - 1, 1)
    txt_size = cv2.getTextSize(display_txt, 0, fontScale=line / 3, thickness=font)[0]
